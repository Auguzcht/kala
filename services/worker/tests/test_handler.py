"""UPDATED for the fourth worker job (tagBackfill). The two tests that
enumerated exactly three jobs now enumerate four, in the handler's run order
(reconcile, embed, tag, readiness). Everything else is unchanged from the
repo's existing test_handler.py — diff this against your copy before dropping
it in, so you can see it is only the job-list additions.
"""
...
def test_handler_runs_all_jobs_and_threads_scope(monkeypatch) -> None:
    ...
    # the handler's declared order (embed before tag: tag reads chunks embed
    # may have just backfilled).
    assert calls == [
...
def test_handler_with_no_event_scopes_to_every_institution(monkeypatch) -> None:
    ...
    assert seen_institution_ids == [None, None, None, None]
