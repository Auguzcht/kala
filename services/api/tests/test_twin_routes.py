...
def test_course_meta_returns_title(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        courses.db, "select",
        lambda table, params: [{"id": "00000000-0000-4000-8000-000000000001", "title": "Thermodynamics I", "lms_course_id": "ME301"}],
    )

    try:
        with TestClient(app) as client:
            response = client.get("/courses/00000000-0000-4000-8000-000000000001")
    finally:
        app.dependency_overrides.clear()

    assert response.status_code == 200
    assert response.json() == {"id": "00000000-0000-4000-8000-000000000001", "title": "Thermodynamics I", "lmsCourseId": "ME301"}
...
def test_twin_returns_skills_bands_and_evidence(monkeypatch) -> None:
    app.dependency_overrides[get_current_user] = authenticated_user
    skills = [
        {"id": "00000000-0000-4000-8000-000000000010", "name": "Cycle Analysis", "bloom_level": "apply", "module_ref": "Module 1"},
        {"id": "00000000-0000-4000-8000-000000000011", "name": "Entropy", "bloom_level": "understand", "module_ref": None},
    ]
    mastery = [{"skill_id": "00000000-0000-4000-8000-000000000010", "estimate": 0.5, "attempts": 3}]
    events = [
        {"id": "ev-1", "skill_id": "00000000-0000-4000-8000-000000000010", "type": "practice", "correct": True,
    ...
    try:
        with TestClient(app) as client:
            response = client.get("/courses/00000000-0000-4000-8000-000000000001/twin")
    finally:
        app.dependency_overrides.clear()
...
def test_twin_empty_when_course_has_no_skills(monkeypatch) -> None:
    ...
    assert response.status_code == 200
    assert response.json() == {"courseId": "00000000-0000-4000-8000-000000000001", "readiness": None, "skills": [], "evidence": []}
...
def test_next_up_picks_weakest_skill_with_reason(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {"id": "00000000-0000-4000-8000-000000000010", "name": "Cycle Analysis", "bloom_level": "apply"},
    )
    monkeypatch.setattr(
        twin.db, "select",
        lambda table, params: [{"skill_id": "00000000-0000-4000-8000-000000000010", "estimate": 0.3}],
    )

    try:
        with TestClient(app) as client:
            response = client.get("/courses/00000000-0000-4000-8000-000000000001/next-up")
    finally:
        app.dependency_overrides.clear()
...
def test_next_up_none_when_course_has_no_skills(monkeypatch) -> None:
    ...
    assert response.status_code == 200
    assert response.json() == {"courseId": "00000000-0000-4000-8000-000000000001", "next": None}
